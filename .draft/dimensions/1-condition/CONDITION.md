...
| A13 | What happens to a Matrix when the method changes under it - must it migrate, may it stay, or does it become invalid? | **OPEN.** Owner requirement stated 2026-07-29: "un protocole d'extraction de la richesse des matrices depreciees pour remplir une plus nouvelle doit être connu". M41 records *which* version produced a Matrix; it does not say what to do about it. Not decided here - owed a D0 pass, like visibility was | Two distinct objects currently share the word *mutation*: a System moving v1 -> v2, and a Matrix moving DRAFT 0.69 -> 0.71. They need separate names before either can be specified, exactly as `DRAFT` and `DRAFT Matrix` did (M29). This decides whether Matrices age well or rot, which is the substance of the owner's heritability goal |
